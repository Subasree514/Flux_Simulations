...
def pareto_analysis(model, objective1=objective1, objective2=objective2, pareto_range=pareto_range, metric=metric,primary=primary):
    ...
    for pareto in np.arange(pareto_range[0], pareto_range[1], pareto_step_size):
        ...
        if metric == 'manhattan':
            ...
            primary=[objective1,objective2,'RIBULOSE_BISPHOSPHATE_CARBOXYLASE_RXN_p','RXN_961_p','TRANSALDOL_RXN_p','1TRANSKETO_RXN_p','SEDOHEPTULOSE_BISPHOSPHATASE_RXN_p','3.2.1.48_RXN_c','SUCROSE_SYNTHASE_RXN_c','GLYCERATE_GLYCOLLATE_pc','1.1.1.39_RXN_m','RXN0_5224_m','PYRUVDEH_RXN_p']
            #print(solution.fluxes[primary].transpose)
            solution_primary.append(solution.fluxes[primary])
...
#print(data.iloc[[1,50,100],np.arange(13)])

barWidth = 0.25
bars1 = data.iloc[0,[5,6,7]]
bars2 = data.iloc[50,[5,6,7]]
bars3 = data.iloc[100,[5,6,7]]
print(bars1)
print(bars2)
print(bars3)
# Bar positions
...
ax.bar(r, bars1, color='#7f6d5f', width=barWidth, edgecolor='white', label='zero')
ax.bar(r2, bars2, color='#557f2d', width=barWidth, edgecolor='white', label='half max')
ax.bar(r3, bars3, color='#2d7f5e', width=barWidth, edgecolor='white', label='max')

# Xticks
ax.set_xlabel('group', fontweight='bold')
ax.set_xticks(r + barWidth)
#primary=[objective1,objective2,'RIBULOSE_BISPHOSPHATE_CARBOXYLASE_RXN_p','RXN_961_p','TRANSALDOL_RXN_p','1TRANSKETO_RXN_p','SEDOHEPTULOSE_BISPHOSPHATASE_RXN_p','3.2.1.48_RXN_c','SUCROSE_SYNTHASE_RXN_c','GLYCERATE_GLYCOLLATE_pc','1.1.1.39_RXN_m','RXN0_5224_m','PYRUVDEH_RXN_p']
#ax.set_xticklabels([primary[2],primary[3],primary[4]],rotation=10)
# Legend and show
ax.legend()
plt.show()
